...
class Crawel():
    ...
    def get_crawler():
        ...
        p1_rank = p1_up_rank + p1_down_rank
        p2_rank = p2_up_rank + p2_down_rank
        ...
        for j in [p1_rank, p2_rank, p3_rank]:
            point_list = []
        ...
        for k in [p1_point, p2_point, p3_point]:
            ...
            if w_2 == 1:
                sum_p1_point += sum_point
                total_p1_point += total_point
            elif w_2 == 2:
                sum_p2_point += sum_point
                total_p2_point += total_point
            elif w_2 == 3:
                sum_p3_point += sum_point
                total_p3_point += total_point
        adjust_factor += 100 / (total_p1_point+total_p2_point+total_p3_point)
        final_sum_point += (sum_p1_point+sum_p2_point+sum_p3_point) * adjust_factor

  # ===========算p1各項總分==========
        # 其他財務指標
        p1_other_sum_point = 0
        p1_other_total_point = 0
        for i in p1_point[0:7]:
            if i != 'NA':
                p1_other_sum_point += i * adjust_factor
                p1_other_total_point += section_point[0] * adjust_factor
        # 財務結構指標
        p1_finance_sum_point = 0
        p1_finance_total_point = 0
        for i in p1_point[7:9]:
            if i != 'NA':
                p1_finance_sum_point += i * adjust_factor
                p1_finance_total_point += section_point[0] * adjust_factor
        # 償還能力指標
        p1_payback_sum_point = 0
        p1_payback_total_point = 0
        for i in p1_point[9:12]:
            if i != 'NA':
                p1_payback_sum_point += i * adjust_factor
                p1_payback_total_point += section_point[0] * adjust_factor
        # 經營能力指標
        p1_business_sum_point = 0
        p1_business_total_point = 0
        for i in p1_point[12:18]:
            if i != 'NA':
                p1_business_sum_point += i * adjust_factor
                p1_business_total_point += section_point[0] * adjust_factor
        # 獲利能力指標
        p1_earning_sum_point = 0
        p1_earning_total_point = 0
        for i in p1_point[18:23]:
            if i != 'NA':
                p1_earning_sum_point += i * adjust_factor
                p1_earning_total_point += section_point[0] * adjust_factor
        # 現金流量指標
        p1_cashflow_sum_point = 0
        p1_cashflow_total_point = 0
        for i in p1_point[23:26]:
            if i != 'NA':
                p1_cashflow_sum_point += i * adjust_factor
                p1_cashflow_total_point += section_point[0] * adjust_factor

  # =========算p2各項總分==========
        # 其他財務指標
        p2_other_sum_point = 0
        p2_other_total_point = 0
        for i in p2_point[0:6]:
            if i != 'NA':
                p2_other_sum_point += i * adjust_factor
                p2_other_total_point += section_point[1] * adjust_factor
        # 財務結構指標
        p2_finance_sum_point = 0
        p2_finance_total_point = 0
        for i in p2_point[6:8]:
            if i != 'NA':
                p2_finance_sum_point += i * adjust_factor
                p2_finance_total_point += section_point[1] * adjust_factor
        # 償還能力指標
        p2_payback_sum_point = 0
        p2_payback_total_point = 0
        for i in p2_point[8:11]:
            if i != 'NA':
                p2_payback_sum_point += i * adjust_factor
                p2_payback_total_point += section_point[1] * adjust_factor
        # 經營能力指標
        p2_business_sum_point = 0
        p2_business_total_point = 0
        for i in p2_point[11:17]:
            if i != 'NA':
                p2_business_sum_point += i * adjust_factor
                p2_business_total_point += section_point[1] * adjust_factor
        # 獲利能力指標
        p2_earning_sum_point = 0
        p2_earning_total_point = 0
        for i in p2_point[17:22]:
            if i != 'NA':
                p2_earning_sum_point += i * adjust_factor
                p2_earning_total_point += section_point[1] * adjust_factor
        # 現金流量指標
        p2_cashflow_sum_point = 0
        p2_cashflow_total_point = 0
        for i in p2_point[22:25]:
            if i != 'NA':
                p2_cashflow_sum_point += i * adjust_factor
                p2_cashflow_total_point += section_point[1] * adjust_factor

        print(co_id)
        print('公司名稱：' + c.co_name)
        print('營業額：' + c.revenue + '億')
        print('資本額：' + c.capitalamount)
        print('產業別：' + c.industry)

        print(final_sum_point)
        print('%.1f' % (sum_p1_point * adjust_factor))
        print('%.1f' % (sum_p2_point * adjust_factor))
        print('%.1f' % (sum_p3_point * adjust_factor))

        print(p1_rank)
        print(p2_rank)
        print(p3_rank)

        print(str('%.1f' % p1_finance_sum_point) + '/' + str('%.1f' % p1_finance_total_point))
        print(str('%.1f' % p1_payback_sum_point) + '/' + str('%.1f' % p1_payback_total_point))
        print(str('%.1f' % p1_business_sum_point) + '/' + str('%.1f' % p1_business_total_point))
        print(str('%.1f' % p1_earning_sum_point) + '/' + str('%.1f' % p1_earning_total_point))
        print(str('%.1f' % p1_cashflow_sum_point) + '/' + str('%.1f' % p1_cashflow_total_point))
        print(str('%.1f' % p1_other_sum_point) + '/' + str('%.1f' % p1_other_total_point))

        print(str('%.1f' % p2_finance_sum_point) + '/' + str('%.1f' % p2_finance_total_point))
        print(str('%.1f' % p2_payback_sum_point) + '/' + str('%.1f' % p2_payback_total_point))
        print(str('%.1f' % p2_business_sum_point) + '/' + str('%.1f' % p2_business_total_point))
        print(str('%.1f' % p2_earning_sum_point) + '/' + str('%.1f' % p2_earning_total_point))
        print(str('%.1f' % p2_cashflow_sum_point) + '/' + str('%.1f' % p2_cashflow_total_point))
        print(str('%.1f' % p2_other_sum_point) + '/' + str('%.1f' % p2_other_total_point))

        print('%.1f' % (p3_point[0] * adjust_factor), '%.1f' % (p3_point[1] * adjust_factor),
              '%.1f' % (p3_point[2] * adjust_factor))
...
class Window1(tk.Frame):
    ...
    #Build Object/建立物件
        ...
        self.profitability = tk.Label(self, height=6, width=58, bg='red', text="\t\t獲利能力\n\t\t------------------------------\n\t\t產業              期間\n\n\t\t1/k               3/n")
        self.fin_structure = tk.Label(self, height=6, width=58, bg='red', text="\t\t財務結構\n\t\t------------------------------\n\t\t產業              期間\n\n\t\t1/k               3/n")
        self.profitability2 = tk.Label(self, height=6, width=58, bg='red', text="\t\t獲利能力\n\t\t------------------------------\n\t\t短\t中\t長\n\n\t\t1/k\t3/k\t1\k")
        ...
        self.management_capacity = tk.Label(self, height=6, width=60, bg='red', text="\t\t\t\t   經營能力\n\t\t\t\t   ------------------------------\n\t\t\t\t   產業              期間\n\n\t\t\t\t   1/k               3/n")
        self.debt = tk.Label(self, height=6, width=30, bg='blue', text="債償能力\n------------------------------\n產業              期間\n\n1/k               3/n")
        self.others = tk.Label(self, height=6, width=60, bg='red', text="\t\t\t\t   其他指標\n\t\t\t\t   ------------------------------\n\t\t\t\t   產業              期間\n\n\t\t\t\t   1/k               3/n")
        self.cash_flow = tk.Label(self, height=6, width=30, bg='blue', text="現金流量\n------------------------------\n產業              期間\n\n1/k               3/n")
        self.final_rank = tk.Label(self, height=6, width=30, bg='blue', text="總排名\n------------------------------\n產業              期間\n\n1/k               3/n")
        self.btn1 = tk.Button(self, height=4, width=32, text="Btn1",command=self.run_btn1)
        self.btn2 = tk.Button(self, height=3, width=16, text="Btn2", command= self.run_secondpage) # 觸發視窗
